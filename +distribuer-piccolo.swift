...
let DISTRIBUTION_DIR = scriptDir + "/../PICCOLO_DISTRIBUTION_" + VERSION_PICCOLO
...
//-------------------- Importer Piccolo
let PICCOLO_DIR = "piccolo-dev"
runCommand ("/bin/rm", ["-fr", PICCOLO_DIR])
runCommand (
  "/usr/bin/git",
...
//  Swift.print ("sha \(sha)")
//-------------------- Écrire le SHA
  let fileRelativePath = "xcode-project/Base.lproj/Credits.rtf"
...
//-------------------- Obtenir le numéro de build
  let plistFileFullPath = DISTRIBUTION_DIR + "/" + PICCOLO_DIR + "/xcode-project/Info.plist"
...
//--- Mettre à jour les numéros de version dans la plist
  plistDictionary ["CFBundleVersion"] = VERSION_PICCOLO + ", " + dateFormatter.string (from: dateConstruction)
  plistDictionary ["CFBundleShortVersionString"] = VERSION_PICCOLO
...
//-------------------- Compiler le projet Xcode
  fm.changeCurrentDirectoryPath (DISTRIBUTION_DIR + "/" + PICCOLO_DIR + "/xcode-project")
...
//  let nomJSON = DISTRIBUTION_DIR + "/" + PRODUCT_NAME + "-" + VERSION_PICCOLO + ".json"
...
//--- Vérifier la signature
  let argumentsSignatureCode = [
    "-dv",
    "--verbose=4",
    DISTRIBUTION_DIR + "/" + PICCOLO_DIR + "/xcode-project/build/Default/" + PRODUCT_NAME + ".app"
  ]
  runCommand ("/usr/bin/codesign", argumentsSignatureCode)
//--- Supprimer les répertoires intermédiaires
  fm.changeCurrentDirectoryPath (DISTRIBUTION_DIR)
  while fm.fileExists (atPath: DISTRIBUTION_DIR + "/" + PICCOLO_DIR) {
    runCommand ("/bin/rm", ["-fr", DISTRIBUTION_DIR + "/" + PICCOLO_DIR])
  }
  //---
