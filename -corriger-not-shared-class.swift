...
//--------------------------------------------------------------------------------------------------
  ...
  //-------------------- Compiler
    ...
    while loop {
      print ("----------------------------------------------------------------------------------------------------------")
      let (s, status) = runHiddenCommand ("/usr/local/bin/galgas", ["--error-on-not-shared-class-declaration", "-q", "--no-color", "--max-errors=1", inCheminFichierGALGAS])
      if status == 0 {
        ...
        print (line1)
        print (line2)
        loop = line2.hasPrefix ("semantic error #1: not shared")
        if loop {
          ...
          ligneConcernée.insert (contentsOf: "shared ", at: idx)
