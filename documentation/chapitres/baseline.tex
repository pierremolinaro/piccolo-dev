...
~\\
\textbf{Routine et BSR.} Un problème important en Piccolo \emph{baseline} est la gestion des pages de la ROM (voir à §§).
...
~\\
\textbf{Appel d’une routine régulière.} Utiliser \texttt{CALL} comme indiqué à §§.

\subsection{Routine sans retour}

L’exécution ne revient pas jamais à l’appelant. Ce type de routine doit donc se terminer par des constructions particulières qui assurent le non-retour : une boucle infinie, un branchement \texttt{bra} vers une autre routine sans retour.
...
Voici leur liste avec les liens vers les sections appropriées :\begin{itemize}
  \item \texttt{BTFSC}, \texttt{BTFSS}, \texttt{CPFSEQ}, \texttt{DECFSZ}, \texttt{DCFSNZ} et \texttt{INCFSZ} : ces instructions sont engendrées par l’instruction conditionnelle simple, l’instruction conditionnelle structurée et l’instruction répétitive ;
