...
\textbf{Attention !} La valeur de \piccolo{ROM\_SIZE} est le nombre d'octets pour un \emph{pic18} (une instruction occupe 2 ou 4 octets), et le nombre d'instructions de 14 bits pour un \emph{mid-range}.

La constante \piccolo{BOOTLOADER\_SIZE} n’est prédéfinie que pour l’implémentation d’un bootloader ou d’un programme applicatif d'un \emph{pic18}. Elle n’est pas définie pour un programme monolithique. Elle a pour valeur la taille en octets réservée pour l’implémentation du bootloader.

La constante \piccolo{RAM\_SIZE} permet d'écrire un code générique pour \emph{pic18} réalisant l'initialisation à zéro de la RAM.



\subsectionLabel{Exemple d'utilisation de \texttt{ROM\_SIZE}}{exempleLTBLPTR-ROMSIZE}

La constante \piccolo{ROM\_SIZE} est définie pour un \emph{pic18}. Associée avec l'instruction \piccolo{ltblptr}, elle permet d'écrire une routine générique, valable pour tous les types de \emph{pic18}, qui renvoie dans \piccolo{WREG}, la somme des octets de la flash.
...
La constante \piccolo{RAM\_SIZE} est définie pour un \emph{pic18}. Elle permet l'écriture d'une séquence générique mettant à zéro toute la RAM d'un micro-contrôleur. On peut faire figurer cette séquence au début de la routine \emph{main}.
...
L'expression litérale présente la syntaxe suivante : elle est introduite par le délimiteur \piccolo{@}, suivi d’un nom de registre, puis, entre parenthèses, une séquence de couples formés d’un nom de champ et d’une valeur immédiate :
\begin{lstlisting}[language=piccolo]
  @ nom\_registre (nom_champ:expression_literale, ...)
\end{lstlisting}

Tout registre spécial, toute variable déclarée (dans une section \piccolo{ram}) ayant des champs définis peut être utilisée.

Le nom de champ (\piccolo{nom\_champ}) est l’un des champs définis par ce registre ou par cette variable. On ne peut utiliser des noms de champ d’un autre registre ou d’une autre variable.

L’\piccolo{expression\_literale} associée obéit à la syntaxe générale des expressions litérales. La valeur obtenue doit être positive ou nulle, et inférieure ou égal à la valeur maximum que peut contenir le champ (c’est à dire, pour un champ de \piccolo{n} bits, $2^n-1$).
