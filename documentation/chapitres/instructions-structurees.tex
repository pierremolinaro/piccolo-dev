...
\begin{itemize}
  ...
  \item l'instruction \piccolo{switch} (\refSectionPage{instructionSwitch}, pour \emph{pic18} uniquement) ;
...
Syntaxiquement, cette instruction accepte une ou plusieurs branches \piccolo{case}, et zéro ou une branche \piccolo{else}. L'absence de branche \piccolo{else} est équivalent à une branche \piccolo{else} vide.

Cette instruction compare successivement le contenu de \piccolo{W} à différentes expressions constantes (ici \piccolo{cst1} puis \piccolo{cst2}). Ces expressions constantes doivent être comprises entre $0$ et $255$. Dès qu'une comparaison avec \piccolo{W} détecte une égalité, la liste d'instructions correspondante est exécutée. Si aucune comparaison ne détecte d'égalité, la liste d'instructions qui suit le \piccolo{else} est exécutée. L'organigramme correspondant à l'exécution de l'instruction \piccolo{switch} ci-dessus est donné par la \refFigure{}{organigrammeSwitchW}.
...
\textbf{Notes :}
\begin{itemize}
  \item les comparaisons successives sont réalisées à l'aide d'instructions \assembleur{ADDLW}, aussi la valeur de \piccolo{W} est modifiée durant les tests successifs; on ne peut donc pas prévoir sa valeur de manière fiable au début des branches \piccolo{case} et de la branche \piccolo{else} ;
...
Il est possible de citer plusieurs constantes dans une branche \piccolo{case} :
\begin{lstlisting}[language=piccolo]
switch w
case cst1 :
  instructions1 # Executees si W == cst1
...
Les instructions associées à une branche \piccolo{case} sont exécutées si \piccolo{W} est égal à l'une des constantes citées. Le coût est deux instructions assembleur par constante citée.


\subsection{Citer un intervalle dans une branche \texttt{case}}

Il est possible de citer un intervalle dans une branche \piccolo{case} :
\begin{lstlisting}[language=piccolo]
switch w
case [cst1, cst2] :
...
L'intervalle doit être valide, c'est à dire \piccolo{cst1 < cst2}. Les instructions associées à cette branche \piccolo{case} sont exécutées si \piccolo{W} est dans l'intervalle cité. Le coût est quatre instructions assembleur : deux instructions \assembleur{ADDLW}, et deux instructions \assembleur{BC} ou  \assembleur{BNC}. Il est donc plus rentable de citer un intervalle plutôt que les constantes une par une.
