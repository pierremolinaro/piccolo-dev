...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{5cm}lll}
    ...
    \hdashline
    Définition de routine mid-range & \pic!routine! & \refSectionPage{routineMidrange}\\
    \hdashline
    Définition de routine sans retour & \pic!noreturn routine!\index{Mot réservé!noreturn} & \refSectionPage{routineMidrange}\\
    \hdashline
    Routine inutilisée & \pic!unused routine!\index{Mot réservé!unused} & \refSubsectionPage{routineInutiliseeMidrange} \\
    \hdashline
    Définition de routine d'interruption mid-range & \pic!interrupt! & \refSectionPage{routineInterruptionMidrange}\\
  \end{tabular}
...
\textbf{Liste d’instructions d’une routine.} Elle est structurée : Piccolo définit des instructions de sélection et de répétition : cela signifie que vous ne pouvez pas déclarer d’étiquette, ni utiliser des \pic!goto! pour effectuer des branchements à l’intérieur d’une routine.
...
\textbf{Routine et pages de la mémoire programme.} La mémoire programme d’un midrange est divisé en blocs de 2048 instructions, et franchir ces frontières posent des problèmes particuliers. Piccolo impose les contraintes suivantes :\begin{itemize}
  ...
  \item les sauts inter-pages ne peuvent être effectués qu’avec les instructions \pic!jump! ou \pic!jsr! (\refSectionPage{gestionPagesMidRange}).
\end{itemize}
...
\textbf{Appel d’une routine sans retour.} Utiliser \pic!goto! ou \pic!jump!.
...
Le compilateur Piccolo insère à l’adresse zéro une instruction \pic!goto! vers cette routine, de façon que la routine \pic!main! soit exécutée au démarrage du micro-contrôleur. Le qualificatif \pic!bank:requires 0! est exigé par le compilateur Piccolo car les bits \assembleur{RP} du registre \assembleur{STATUS} sont initialisés à zéro au démarrage du micro-contrôleur.
...
\begin{itemize}
  ...
  \item l'instruction d'appel de routine régulière \pic!jsr! (\refSectionPage{gestionPagesMidRange}) ;
  \item l'instruction d'appel de routine sans retour \pic!jump! (\refSectionPage{gestionPagesMidRange}) ;
...
\subsectionLabel{Instructions nommant un registre, et optionellement \texttt{W}}{instructionsMidRangeNommantRegistreEtW}
...
\begin{itemize}
  ...
  \item \assembleur{d} : optionnel en Piccolo ; si absent, le registre \pic!f! est destination de l'opération, si égal à \pic!W!, c'est le registre \pic!W! qui est destinaire.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \caption{Instructions \emph{mid-range} nommant un registre, et optionellement \texttt{W}}
...
Une routine régulière est une routine déclarée sans le qualificatif \pic!noreturn!. L'appel s’effectue avec l'instruction \pic!call!, si la routine se trouve dans la même page de la mémoire programme que l'instruction d'appel, ou avec l'instruction \pic!jsr! (\refSubsectionPage{instructionJsrMidRange}).
...
Appeler une routine sans retour (c'est-à-dire déclarée avec le qualificatif \pic!noreturn!) s'effectue par une instruction \pic!goto!, si la routine se trouve dans la même page de la mémoire programme que l'instruction d'appel, ou avec l'instruction \pic!jump! (\refSubsectionPage{instructionJumpMidRange}). L'appel s'écrit :
\begin{piccolo}
goto nom_routine
\end{piccolo}
...
Une section \pic!include! permet d'inclure un fichier contenant lui-même des sections telles que définies dans le \refTableauPage{sectionsMidrange}.  Son format est le suivant :
...
En Piccolo, il est déconseillé d'accéder directement à \assembleur{PCLATH}. Les instructions \pic!jsr! et \pic!jump! prennent en charge les branchements vers une autre page. En résumé :
\begin{itemize}
  ...
  \item pour appeler des routines placées dans une autre page, utiliser les instructions Piccolo \pic!jsr! (\refSubsectionPage{instructionJsrMidRange}) et \pic!jump! (\refSubsectionPage{instructionJumpMidRange}).
\end{itemize}
...
L'instruction Piccolo \pic!jump! engendre des instructions machine \assembleur{BCF} ou \assembleur{BSF} pour modifier les bits 3 et 4 du registre \assembleur{PCLATH}, puis l'instruction machine \assembleur{GOTO}.
...
L'instruction Piccolo  \pic!jsr! engendre des instructions machine \assembleur{BCF} ou \assembleur{BSF} pour modifier les bits 3 et 4 du registre \assembleur{PCLATH}, puis l'instruction machine \assembleur{CALL}, puis de nouveau des instructions machine \assembleur{BCF} ou \assembleur{BSF} pour rétablir les bits 3 et 4 du registre \assembleur{PCLATH}.
...
Où \pic!numero_page! est un entier positif ou nul strictement inférieur au nombre de pages. Par défaut, si la page n'est pas déclarée, la routine est placée dans la page zéro. La déclaration de la page doit précéder la déclaration de banc \pic!bank!.
...
L'instruction assembleur \assembleur{RETFIE} n'existe pas en Piccolo, le compilateur l'ajoute automatiquement. Le sauvegarde du contexte engendré par Piccolo assure que le banc 0 est sélectionné pour l'exécution de \pic!instructions!.
...
\begin{itemize}
  ...
  \item \assembleur{STATUS\_TEMP} pour sauver \assembleur{STATUS} ; cette variable doit être déclarée dans le banc 0, qui s'appelle générallement \pic!gpr0!. 
\end{itemize}
...
\begin{itemize}
  \item \assembleur{W\_TEMP}, de dimension 1, pour sauver \assembleur{W} ; cette variable doit être déclarée dans la zone commune à tous les bancs ; celle-ci s'appelle générallement \pic!gprnobnk! ; 
  \item \assembleur{STATUS\_PCLATH\_TEMP}, de dimension 2, pour sauver \assembleur{STATUS} et  \assembleur{PCLATH} ; cette variable doit être déclarée dans le banc 0, qui s'appelle générallement \pic!gpr0!. 
\end{itemize}
...
\begin{itemize}
  \item la première \assembleur{W\_STATUS\_TEMP}, de dimension 2, pour sauver \assembleur{W} et \assembleur{STATUS} ; cette variable doit être déclarée doit être déclarée dans le banc 0, qui s'appelle générallement \pic!gpr0! ; 
  \item \assembleur{W\_TEMP\_1} pour sauver \assembleur{W}, de dimension 1 ; cette variable doit être déclarée dans le banc 1, qui s'appelle générallement \pic!gpr1!.
...
On déclare donc deux variables :
\begin{itemize}
  \item la première \assembleur{W\_STATUS\_PCLATH\_TEMP}, de dimension 3, pour sauver \assembleur{W}, \assembleur{STATUS} et \assembleur{PCLATH} ; cette variable doit être déclarée doit être déclarée dans le banc 0, qui s'appelle générallement \pic!gpr0! ; 
  \item \assembleur{W\_TEMP\_1} pour sauver \assembleur{W}, de dimension 1 ; cette variable doit être déclarée dans le banc 1, qui s'appelle générallement \pic!gpr1!.
