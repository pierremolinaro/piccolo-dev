...
void routine_pic18_build_assembly_code (C_Compiler & inLexique,
  ...
  GGS_uint  var_cas_bccLabelIndex = GGS_uint (0U) ;
  {
    ...
    while (((operand_14919 = enumerator_14919.nextObject ()))) {
      ...
      { typeCategoryMethod__ipic18_instruction__buildAssemblyCode method = findCategoryMethod__ipic18_instruction__buildAssemblyCode (operand_14919->mInstruction.galgasObjectRunTimeInfo ()) ;
        if (method != NULL) {
          method (inLexique, operand_14919->mInstruction (HERE), var_cas_lineList, var_cas_bccLabelIndex COMMA_SOURCE_FILE_AT_LINE (513)) ;
        }
      }
      {
  ...
  var_cas_outAssemblyCode.appendCString (";--- Prefefined registers\n") ;
  {
    ...
    while (((operand_15263 = enumerator_15263.nextObject ()))) {
      ...
      if (operand_15263->mInfo.mRegisterAddressList.isBuilt ()) {
        operand_15263->mInfo.mRegisterAddressList (HERE)->method_first (inLexique, var_cas_firstRegister COMMA_SOURCE_FILE_AT_LINE (522)) ;
      }
  ...
  {
    GGS_registerTable::cEnumerator enumerator_15590 (var_cas_inAllRegisters, true) ;
    const GGS_registerTable::cElement * operand_15590 = NULL ;
    while (((operand_15590 = enumerator_15590.nextObject ()))) {
      macroValidPointer (operand_15590) ;
      const GGS_bool cond_15823 = (var_cas_inPredefinedRegisters.reader_hasKey (operand_15590->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (529)))).operator_not () ;
      if (cond_15823.isBuiltAndTrue ()) {
        GGS_uint  var_cas_firstRegister ;
        if (operand_15590->mInfo.mRegisterAddressList.isBuilt ()) {
          operand_15590->mInfo.mRegisterAddressList (HERE)->method_first (inLexique, var_cas_firstRegister COMMA_SOURCE_FILE_AT_LINE (530)) ;
        }
        var_cas_outAssemblyCode.dotAssign_operation (operand_15590->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (531))) ;
        var_cas_outAssemblyCode.appendCString (" EQU ") ;
  ...
  var_cas_outAssemblyCode.appendCString ("BSR_ACCESS EQU 1\n"
    "\n") ;
  const GGS_bool cond_16372 = (var_cas_inActualConfigurationMap.reader_count ()).operator_strictSup (GGS_uint (0U)) ;
  if (cond_16372.isBuiltAndTrue ()) {
    var_cas_outAssemblyCode.appendCString (";--- Configuration\n"
      "\n") ;
    {
      GGS_actualConfigurationMap::cEnumerator enumerator_16224 (var_cas_inActualConfigurationMap, true) ;
      const GGS_actualConfigurationMap::cElement * operand_16224 = NULL ;
      while (((operand_16224 = enumerator_16224.nextObject ()))) {
        ...
        var_cas_outAssemblyCode.dotAssign_operation (operand_16224->mInfo.mRegisterAddress.reader_hexString (inLexique COMMA_SOURCE_FILE_AT_LINE (543))) ;
        var_cas_outAssemblyCode.appendCString (", ") ;
        var_cas_outAssemblyCode.dotAssign_operation ((operand_16224->mInfo.mRegisterValue).operator_and (GGS_uint (255U)).reader_hexString (inLexique COMMA_SOURCE_FILE_AT_LINE (543))) ;
        var_cas_outAssemblyCode.appendCString ("\n") ;
      }
    }
  }
