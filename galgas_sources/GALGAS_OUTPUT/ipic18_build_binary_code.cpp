...
void routine_build_ipic18_binary_code (C_Compiler & _inLexique,
  ...
  {
    GGS_ipic18_instructionList::cEnumerator enumerator_24867 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic18_instructionList::cElement * operand_24867 = NULL ;
    while (((operand_24867 = enumerator_24867.nextObject ()))) {
      macroValidPointer (operand_24867) ;
      const GGS_ipic18_instruction  _temp_24908 = operand_24867->mInstruction ;
      if (_temp_24908._isBuilt ()) {
        typeCategoryMethod__ipic18_instruction__enterLabelAtAddress _method = findCategoryMethod__ipic18_instruction__enterLabelAtAddress (_temp_24908._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_24908 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (763)) ;
        }
      }
    }
  }
  ...
  var_cas_currentAddress = GGS_uint (true, 0U) ;
  if (GGS_uint (true, 0U)._isBuilt ()) {
    ::routine_setEmitAddress (_inLexique,  GGS_uint (true, 0U) COMMA_SOURCE_FILE_AT_LINE (769)) ;
  }
  {
    GGS_ipic18_instructionList::cEnumerator enumerator_25191 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic18_instructionList::cElement * operand_25191 = NULL ;
    while (((operand_25191 = enumerator_25191.nextObject ()))) {
      macroValidPointer (operand_25191) ;
      const GGS_ipic18_instruction  _temp_25240 = operand_25191->mInstruction ;
      if (_temp_25240._isBuilt ()) {
        typeCategoryMethod__ipic18_instruction__generateBinaryCodeAtAddress _method = findCategoryMethod__ipic18_instruction__generateBinaryCodeAtAddress (_temp_25240._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_25240 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (771)) ;
        }
      }
      GGS_uint  var_cas_instructionLength ;
      const GGS_ipic18_instruction  _temp_25339 = operand_25191->mInstruction ;
      if (_temp_25339._isBuilt ()) {
        _temp_25339 (HERE)->method_instructionLength (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (773)) ;
      }
      var_cas_currentAddress = var_cas_currentAddress._add_operation (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (774)) ;
      ::routine_checkCurrentEmitAddress (_inLexique,  var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (775)) ;
    }
  }
  if (((var_cas_lastAddressForFirstPass)._operator_isNotEqual (var_cas_currentAddress)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Internal second pass error: the last address gets "))._operator_concat (var_cas_currentAddress.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (778))))._operator_concat (GGS_string (true, " value in second pass, while it gets ")))._operator_concat (var_cas_lastAddressForFirstPass.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (780))))._operator_concat (GGS_string (true, " value in first pass")) COMMA_SOURCE_FILE_AT_LINE (781)) ;
  }else if (((var_cas_lastAddressForFirstPass)._operator_strictSup (var_cas_inROMsize)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Program too large: "))._operator_concat (var_cas_lastAddressForFirstPass.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (782))))._operator_concat (GGS_string (true, " bytes (rom size is ")))._operator_concat (var_cas_inROMsize.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (784))))._operator_concat (GGS_string (true, " bytes)")) COMMA_SOURCE_FILE_AT_LINE (785)) ;
  }
  {
    GGS_actualConfigurationMap::cEnumerator enumerator_26036 (var_cas_inActualConfigurationMap, true) ;
    const GGS_actualConfigurationMap::cElement * operand_26036 = NULL ;
    while (((operand_26036 = enumerator_26036.nextObject ()))) {
      macroValidPointer (operand_26036) ;
      if (operand_26036->mInfo.mRegisterAddress._isBuilt ()) {
        ::routine_setEmitAddress (_inLexique,  operand_26036->mInfo.mRegisterAddress COMMA_SOURCE_FILE_AT_LINE (788)) ;
      }
      if (operand_26036->mInfo.mRegisterValue._isBuilt ()) {
        ::routine_emitByte (_inLexique,  operand_26036->mInfo.mRegisterValue COMMA_SOURCE_FILE_AT_LINE (789)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
