...
void routine_build_ipic18_binary_code (C_Compiler & _inLexique,
  ...
  {
    GGS_ipic18_instructionList::cEnumerator enumerator_23547 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic18_instructionList::cElement * operand_23547 = NULL ;
    while (((operand_23547 = enumerator_23547.nextObject ()))) {
      macroValidPointer (operand_23547) ;
      const GGS_ipic18_instruction  _temp_23588 = operand_23547->mInstruction ;
      if (_temp_23588._isBuilt ()) {
        typeCategoryMethod__ipic18_instruction__enterLabelAtAddress _method = findCategoryMethod__ipic18_instruction__enterLabelAtAddress (_temp_23588._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_23588 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (731)) ;
        }
      }
    }
  }
  ...
  var_cas_currentAddress = GGS_uint (true, 0U) ;
  if (GGS_uint (true, 0U)._isBuilt ()) {
    ::routine_setEmitAddress (_inLexique,  GGS_uint (true, 0U) COMMA_SOURCE_FILE_AT_LINE (736)) ;
  }
  {
    GGS_ipic18_instructionList::cEnumerator enumerator_23834 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic18_instructionList::cElement * operand_23834 = NULL ;
    while (((operand_23834 = enumerator_23834.nextObject ()))) {
      macroValidPointer (operand_23834) ;
      const GGS_ipic18_instruction  _temp_23883 = operand_23834->mInstruction ;
      if (_temp_23883._isBuilt ()) {
        typeCategoryMethod__ipic18_instruction__generateBinaryCodeAtAddress _method = findCategoryMethod__ipic18_instruction__generateBinaryCodeAtAddress (_temp_23883._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_23883 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (738)) ;
        }
      }
      GGS_uint  var_cas_instructionLength ;
      const GGS_ipic18_instruction  _temp_23982 = operand_23834->mInstruction ;
      if (_temp_23982._isBuilt ()) {
        _temp_23982 (HERE)->method_instructionLength (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (740)) ;
      }
      var_cas_currentAddress = var_cas_currentAddress._add_operation (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (741)) ;
      ::routine_checkCurrentEmitAddress (_inLexique,  var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (742)) ;
    }
  }
  if (((var_cas_lastAddressForFirstPass)._operator_isNotEqual (var_cas_currentAddress)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Internal second pass error: the last address gets "))._operator_concat (var_cas_currentAddress.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (745))))._operator_concat (GGS_string (true, " value in second pass, while it gets ")))._operator_concat (var_cas_lastAddressForFirstPass.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (747))))._operator_concat (GGS_string (true, " value in first pass")) COMMA_SOURCE_FILE_AT_LINE (748)) ;
  }else if (((var_cas_lastAddressForFirstPass)._operator_strictSup (var_cas_inROMsize)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Program too large: "))._operator_concat (var_cas_lastAddressForFirstPass.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (749))))._operator_concat (GGS_string (true, " bytes (rom size is ")))._operator_concat (var_cas_inROMsize.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (751))))._operator_concat (GGS_string (true, " bytes)")) COMMA_SOURCE_FILE_AT_LINE (752)) ;
  }
  {
    GGS_actualConfigurationMap::cEnumerator enumerator_24679 (var_cas_inActualConfigurationMap, true) ;
    const GGS_actualConfigurationMap::cElement * operand_24679 = NULL ;
    while (((operand_24679 = enumerator_24679.nextObject ()))) {
      macroValidPointer (operand_24679) ;
      if (operand_24679->mInfo.mRegisterAddress._isBuilt ()) {
        ::routine_setEmitAddress (_inLexique,  operand_24679->mInfo.mRegisterAddress COMMA_SOURCE_FILE_AT_LINE (755)) ;
      }
      if (operand_24679->mInfo.mRegisterValue._isBuilt ()) {
        ::routine_emitByte (_inLexique,  operand_24679->mInfo.mRegisterValue COMMA_SOURCE_FILE_AT_LINE (756)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
