...
void routine_displayBlockList (const GALGAS_string constinArgument_inTitle,
  ...
  cEnumerator_ipic_31__38_BlockList enumerator_1604 (constinArgument_inGeneratedBlockList, kEnumeration_up) ;
  GALGAS_uint index_1598 ((PMUInt32) 0) ;
  while (enumerator_1604.hasCurrentObject ()) {
    GALGAS_string var_nextBlockLabel ;
    const enumGalgasBool test_0 = GALGAS_bool (kIsStrictInf, index_1598.add_operation (GALGAS_uint ((PMUInt32) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (50)).objectCompare (constinArgument_inGeneratedBlockList.reader_length (SOURCE_FILE_AT_LINE (50)))).boolEnum () ;
    if (kBoolTrue == test_0) {
      var_nextBlockLabel = constinArgument_inGeneratedBlockList.reader_mBlockAtIndex (index_1598.add_operation (GALGAS_uint ((PMUInt32) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (51)), inCompiler COMMA_SOURCE_FILE_AT_LINE (51)).mAttribute_mLabel.mAttribute_string ;
    }else if (kBoolFalse == test_0) {
      var_nextBlockLabel = GALGAS_string::makeEmptyString () ;
    }
    categoryMethod_display (enumerator_1604.current_mBlock (HERE), var_nextBlockLabel, ioArgument_ioListFileContents, inCompiler COMMA_SOURCE_FILE_AT_LINE (55)) ;
    enumerator_1604.gotoNextObject () ;
    index_1598.increment_operation (inCompiler COMMA_SOURCE_FILE_AT_LINE (48)) ;
  }
}
...
void categoryMethod_display (const GALGAS_ipic_31__38_Block inObject,
  ...
  const enumGalgasBool test_0 = GALGAS_bool (kIsNotEqual, inObject.mAttribute_mStartAddress.objectCompare (GALGAS_uint::constructor_max (SOURCE_FILE_AT_LINE (17)))).boolEnum () ;
  if (kBoolTrue == test_0) {
    ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string (", ORG ").operator_concat (inObject.mAttribute_mStartAddress.reader_hexString (SOURCE_FILE_AT_LINE (18)) COMMA_SOURCE_FILE_AT_LINE (18)) COMMA_SOURCE_FILE_AT_LINE (18)) ;
  }
  ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string (":\n") COMMA_SOURCE_FILE_AT_LINE (20)) ;
  cEnumerator_ipic_31__38_SequentialInstructionList enumerator_689 (inObject.mAttribute_mInstructionList, kEnumeration_up) ;
  while (enumerator_689.hasCurrentObject ()) {
    GALGAS_stringlist var_d = callCategoryReader_instructionDisplay ((const cPtr_ipic_31__38_SequentialInstruction *) enumerator_689.current_mInstruction (HERE).ptr (), inCompiler COMMA_SOURCE_FILE_AT_LINE (23)) ;
    cEnumerator_stringlist enumerator_769 (var_d, kEnumeration_up) ;
    while (enumerator_769.hasCurrentObject ()) {
      ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string ("  ").operator_concat (enumerator_769.current_mValue (HERE) COMMA_SOURCE_FILE_AT_LINE (25)).operator_concat (GALGAS_string ("\n") COMMA_SOURCE_FILE_AT_LINE (25)) COMMA_SOURCE_FILE_AT_LINE (25)) ;
