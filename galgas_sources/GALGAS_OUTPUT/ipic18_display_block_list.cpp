...
void routine_displayBlockList (const GALGAS_string constinArgument_inTitle,
  ...
  ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string ("*").operator_concat (constinArgument_inTitle.reader_stringByLeftAndRightPadding (GALGAS_uint ((uint32_t) 77U), GALGAS_char (TO_UNICODE (32)) COMMA_SOURCE_FILE_AT_LINE (46)) COMMA_SOURCE_FILE_AT_LINE (46)).operator_concat (GALGAS_string ("*\n") COMMA_SOURCE_FILE_AT_LINE (46)) COMMA_SOURCE_FILE_AT_LINE (46)) ;
  ...
  while (enumerator_1588.hasCurrentObject ()) {
    ...
    const enumGalgasBool test_0 = GALGAS_bool (kIsStrictInf, index_1567.add_operation (GALGAS_uint ((uint32_t) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (50)).objectCompare (constinArgument_inGeneratedBlockList.reader_length (SOURCE_FILE_AT_LINE (50)))).boolEnum () ;
    if (kBoolTrue == test_0) {
      var_nextBlockLabel = constinArgument_inGeneratedBlockList.reader_mBlockAtIndex (index_1567.add_operation (GALGAS_uint ((uint32_t) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (51)), inCompiler COMMA_SOURCE_FILE_AT_LINE (51)).mAttribute_mLabel.mAttribute_string ;
    }else if (kBoolFalse == test_0) {
      var_nextBlockLabel = GALGAS_string::makeEmptyString () ;
    }
    categoryMethod_display (enumerator_1588.current_mBlock (HERE), var_nextBlockLabel, ioArgument_ioListFileContents, inCompiler COMMA_SOURCE_FILE_AT_LINE (55)) ;
    enumerator_1588.gotoNextObject () ;
    index_1567.increment_operation (inCompiler COMMA_SOURCE_FILE_AT_LINE (48)) ;
  }
}
...
void categoryMethod_display (const GALGAS_ipic_31__38_Block inObject,
  ...
  while (enumerator_676.hasCurrentObject ()) {
    GALGAS_stringlist var_d = callCategoryReader_instructionDisplay ((const cPtr_ipic_31__38_SequentialInstruction *) enumerator_676.current_mInstruction (HERE).ptr (), inCompiler COMMA_SOURCE_FILE_AT_LINE (23)) ;
    cEnumerator_stringlist enumerator_756 (var_d, kEnumeration_up) ;
    while (enumerator_756.hasCurrentObject ()) {
      ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string ("  ").operator_concat (enumerator_756.current_mValue (HERE) COMMA_SOURCE_FILE_AT_LINE (25)).operator_concat (GALGAS_string ("\n") COMMA_SOURCE_FILE_AT_LINE (25)) COMMA_SOURCE_FILE_AT_LINE (25)) ;
