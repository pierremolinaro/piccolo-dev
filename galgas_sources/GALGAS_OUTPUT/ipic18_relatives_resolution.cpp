...
void routine_ipic_31__38_RelativesResolution (GALGAS_string & ioArgument_ioListFileContents,
  ...
  if (ioArgument_ioGeneratedBlockList.reader_length (SOURCE_FILE_AT_LINE (550)).add_operation (GALGAS_uint ((PMUInt32) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (550)).isValid ()) {
    PMUInt32 variant_20712 = ioArgument_ioGeneratedBlockList.reader_length (SOURCE_FILE_AT_LINE (550)).add_operation (GALGAS_uint ((PMUInt32) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (550)).uintValue () ;
    bool loop_20712 = true ;
    while (loop_20712) {
      loop_20712 = var_resolutionDone.operator_not (SOURCE_FILE_AT_LINE (551)).operator_and (GALGAS_bool (kIsEqual, GALGAS_uint::constructor_errorCount (SOURCE_FILE_AT_LINE (551)).objectCompare (GALGAS_uint ((PMUInt32) 0U))) COMMA_SOURCE_FILE_AT_LINE (551)).isValid () ;
      if (loop_20712) {
        loop_20712 = var_resolutionDone.operator_not (SOURCE_FILE_AT_LINE (551)).operator_and (GALGAS_bool (kIsEqual, GALGAS_uint::constructor_errorCount (SOURCE_FILE_AT_LINE (551)).objectCompare (GALGAS_uint ((PMUInt32) 0U))) COMMA_SOURCE_FILE_AT_LINE (551)).boolValue () ;
      }
      if (loop_20712 && (0 == variant_20712)) {
        loop_20712 = false ;
      ...
      if (loop_20712) {
        ...
        {
        GALGAS_uint joker_20947_0 ; // Joker input parameter
        routine_computeLabelAbsoluteAddressMap (ioArgument_ioGeneratedBlockList, var_symbolTable, joker_20947_0, inCompiler COMMA_SOURCE_FILE_AT_LINE (553)) ;
        }
        ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string ("  Pass ").operator_concat (var_pass.reader_string (SOURCE_FILE_AT_LINE (558)) COMMA_SOURCE_FILE_AT_LINE (558)).operator_concat (GALGAS_string (":\n") COMMA_SOURCE_FILE_AT_LINE (558)) COMMA_SOURCE_FILE_AT_LINE (558)) ;
