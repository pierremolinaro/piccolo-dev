...
void routine_addMidRangeUsedRoutinesFromInstructionList (const GALGAS_midrange_5F_instructionList constinArgument_inInstructionList,
  ...
  while (enumerator_6414.hasCurrentObject ()) {
    callCategoryMethod_addUsedRoutines ((const cPtr_midrange_5F_instruction *) enumerator_6414.current_mInstruction (HERE).ptr (), ioArgument_ioUsedRoutines, inCompiler COMMA_SOURCE_FILE_AT_LINE (215)) ;
...
GALGAS_stringset function_midrange_5F_computeUsedRoutines (const GALGAS_midrange_5F_interruptDefinitionList & constinArgument_inInterruptDefinitionList,
  ...
  while (enumerator_6953.hasCurrentObject ()) {
    {
    routine_addMidRangeUsedRoutinesFromInstructionList (enumerator_6953.current_mInstructionList (HERE), result_outUsedRoutineSet, inCompiler COMMA_SOURCE_FILE_AT_LINE (230)) ;
    }
  ...
  if (constinArgument_inRoutineDefinitionList.reader_length (SOURCE_FILE_AT_LINE (237)).isValid ()) {
    uint32_t variant_7153 = constinArgument_inRoutineDefinitionList.reader_length (SOURCE_FILE_AT_LINE (237)).uintValue () ;
    bool loop_7153 = true ;
    while (loop_7153) {
      ...
      if (loop_7153) {
        ...
        while (enumerator_7287.hasCurrentObject ()) {
          const enumGalgasBool test_0 = result_outUsedRoutineSet.reader_hasKey (enumerator_7287.current_mRoutineName (HERE).mAttribute_string COMMA_SOURCE_FILE_AT_LINE (241)).boolEnum () ;
          if (kBoolTrue == test_0) {
            {
            routine_addMidRangeUsedRoutinesFromInstructionList (enumerator_7287.current_mInstructionList (HERE), result_outUsedRoutineSet, inCompiler COMMA_SOURCE_FILE_AT_LINE (242)) ;
            }
          }
