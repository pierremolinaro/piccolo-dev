...
void routine_pic18BootloaderSpecificationAnalysis (C_Compiler & inLexique,
  ...
  var_cas_outRamBank = var_cas_outPiccoloDeviceModel.reader_mRamBankTable (inLexique COMMA_SOURCE_FILE_AT_LINE (48)) ;
  {
    ...
    while (((operand_2655 = enumerator_2655.nextObject ()))) {
      ...
      const GGS_ramBankTable  temp_2812 = var_cas_outRamBank ;
      if (temp_2812.isBuilt ()) {
        temp_2812 (HERE)->method_searchKey (inLexique, operand_2655->mKey, var_cas_firstAddress, var_cas_firstFreeAddress, var_cas_lastAddressPlusOne, var_cas_mirrorOffsetList COMMA_SOURCE_FILE_AT_LINE (54)) ;
      }
      ...
      const GGS_bool cond_3170 = (var_cas_firstFreeAddress).operator_strictSup (var_cas_lastAddressPlusOne) ;
      if (cond_3170.isBuiltAndTrue ()) {
      ...
      }else if (cond_3170.isBuiltAndFalse ()) {
  ...
  {
    GGS_routineDeclarationList::cEnumerator enumerator_4010 (var_cas_inBootloaderRoutineDeclarationList, true) ;
    const GGS_routineDeclarationList::cElement * operand_4010 = NULL ;
    while (((operand_4010 = enumerator_4010.nextObject ()))) {
      macroValidPointer (operand_4010) ;
      const GGS_bool cond_4142 = var_cas_routineNameSet.reader_hasKey (operand_4010->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (73))) ;
      if (cond_4142.isBuiltAndTrue ()) {
        operand_4010->mRoutineName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("This routine is already declared") COMMA_SOURCE_FILE_AT_LINE (75)) ;
      }
      var_cas_routineNameSet.addAssign_operation (operand_4010->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (76))) ;
      var_cas_outListFileContents.dotAssign_operation (operand_4010->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (77)).reader_stringByLeftPadding (GGS_uint (40U), GGS_char (TO_UNICODE (' ')))) ;
      var_cas_outListFileContents.appendCString (" ") ;
      ...
      const GGS_bool cond_4452 = (var_cas_entryPointAddress).operator_isEqual (GGS_uint (8U)) ;
      if (cond_4452.isBuiltAndTrue ()) {
        var_cas_entryPointAddress = GGS_uint (12U) ;
      }else if (cond_4452.isBuiltAndFalse ()) {
        const GGS_bool cond_4525 = (var_cas_entryPointAddress).operator_isEqual (GGS_uint (24U)) ;
        if (cond_4525.isBuiltAndTrue ()) {
  ...
  {
    GGS_routineDeclarationList::cEnumerator enumerator_5082 (var_cas_inUserRoutineDeclarationList, true) ;
    const GGS_routineDeclarationList::cElement * operand_5082 = NULL ;
    while (((operand_5082 = enumerator_5082.nextObject ()))) {
      macroValidPointer (operand_5082) ;
      const GGS_bool cond_5214 = var_cas_routineNameSet.reader_hasKey (operand_5082->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (93))) ;
      if (cond_5214.isBuiltAndTrue ()) {
        operand_5082->mRoutineName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("This routine is already declared") COMMA_SOURCE_FILE_AT_LINE (95)) ;
      }
      var_cas_routineNameSet.addAssign_operation (operand_5082->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (96))) ;
      var_cas_outListFileContents.dotAssign_operation (operand_5082->mRoutineName.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (97)).reader_stringByLeftPadding (GGS_uint (40U), GGS_char (TO_UNICODE (' ')))) ;
      var_cas_outListFileContents.appendCString (" ") ;
