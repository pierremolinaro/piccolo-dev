...
void routine_pic18_analyze (C_Compiler & _inLexique,
  ...
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (115)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_5731 = var_cas_assemblyCode ;
      if (_temp_5731._isBuilt ()) {
        _temp_5731 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_asmDestinationFile COMMA_SOURCE_FILE_AT_LINE (126)) ;
      }
    }
  }else{
    GGS_string var_cas_hexDestinationFile ;
