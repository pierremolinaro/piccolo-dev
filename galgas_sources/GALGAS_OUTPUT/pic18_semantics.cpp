...
void routine_pic18_analyze (C_Compiler & _inLexique,
  ...
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ::routine_convertRelativeBranchsToAbsoluteWhenNeeded (_inLexique,  var_cas_generatedInstructionList,  var_cas_listFileContents COMMA_SOURCE_FILE_AT_LINE (95)) ;
  }
  if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "ouputListingFile" COMMA_SOURCE_FILE_AT_LINE (98)))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_4712 = var_cas_listFileContents ;
    if (_temp_4712._isBuilt ()) {
      _temp_4712 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_listFile COMMA_SOURCE_FILE_AT_LINE (100)) ;
    }
  }
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_5179 = var_cas_contents ;
    if (_temp_5179._isBuilt ()) {
      _temp_5179 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_hexDestinationFile COMMA_SOURCE_FILE_AT_LINE (113)) ;
    }
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (115)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_5739 = var_cas_assemblyCode ;
      if (_temp_5739._isBuilt ()) {
        _temp_5739 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_asmDestinationFile COMMA_SOURCE_FILE_AT_LINE (126)) ;
      }
    }
  }else{
    GGS_string var_cas_hexDestinationFile ;
