...
void routine_buildBinaryCode (C_Compiler & _inLexique,
  ...
  {
    GGS_ipic_instructionList::cEnumerator enumerator_23720 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic_instructionList::cElement * operand_23720 = NULL ;
    while (((operand_23720 = enumerator_23720.nextObject ()))) {
      macroValidPointer (operand_23720) ;
      const GGS_ipic_instruction  _temp_23768 = operand_23720->mInstruction ;
      if (_temp_23768._isBuilt ()) {
        typeCategoryMethod__ipic_instruction__enterLabelAtAddress _method = findCategoryMethod__ipic_instruction__enterLabelAtAddress (_temp_23768._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_23768 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (741)) ;
        }
      }
    }
  }
  ...
  var_cas_currentAddress = GGS_uint (true, 0U) ;
  if (GGS_uint (true, 0U)._isBuilt ()) {
    ::routine_setEmitAddress (_inLexique,  GGS_uint (true, 0U) COMMA_SOURCE_FILE_AT_LINE (745)) ;
  }
  {
    GGS_ipic_instructionList::cEnumerator enumerator_24007 (var_cas_inGeneratedInstructionList, true) ;
    const GGS_ipic_instructionList::cElement * operand_24007 = NULL ;
    while (((operand_24007 = enumerator_24007.nextObject ()))) {
      macroValidPointer (operand_24007) ;
      const GGS_ipic_instruction  _temp_24055 = operand_24007->mInstruction ;
      if (_temp_24055._isBuilt ()) {
        typeCategoryMethod__ipic_instruction__generateBinaryCodeAtAddress _method = findCategoryMethod__ipic_instruction__generateBinaryCodeAtAddress (_temp_24055._galgasObjectRunTimeInfo ()) ;
        if (_method != NULL) {
          _method (_inLexique, _temp_24055 (HERE), var_cas_symbolTable, var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (748)) ;
        }
      }
      GGS_uint  var_cas_instructionLength ;
      const GGS_ipic_instruction  _temp_24164 = operand_24007->mInstruction ;
      if (_temp_24164._isBuilt ()) {
        _temp_24164 (HERE)->method_instructionLength (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (750)) ;
      }
      var_cas_currentAddress = var_cas_currentAddress._add_operation (_inLexique, var_cas_instructionLength COMMA_SOURCE_FILE_AT_LINE (751)) ;
      ::routine_checkCurrentEmitAddress (_inLexique,  var_cas_currentAddress COMMA_SOURCE_FILE_AT_LINE (751)) ;
    }
  }
  if (((var_cas_lastAddressForFirstPass)._operator_isNotEqual (var_cas_currentAddress)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Internal second pass error: the last address gets "))._operator_concat (var_cas_currentAddress.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (755))))._operator_concat (GGS_string (true, " value in second pass, while it gets ")))._operator_concat (var_cas_lastAddressForFirstPass.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (757))))._operator_concat (GGS_string (true, " value in first pass")) COMMA_SOURCE_FILE_AT_LINE (757)) ;
  }else if (((var_cas_lastAddressForFirstPass)._operator_strictSup (var_cas_inROMsize)).isBuiltAndTrue ()) {
    GGS_location (_inLexique).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "Program too large: "))._operator_concat (var_cas_lastAddressForFirstPass.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (759))))._operator_concat (GGS_string (true, " bytes (rom size is ")))._operator_concat (var_cas_inROMsize.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (761))))._operator_concat (GGS_string (true, " bytes)")) COMMA_SOURCE_FILE_AT_LINE (761)) ;
  }
  #ifdef DEBUG_TRACE_ENABLED
