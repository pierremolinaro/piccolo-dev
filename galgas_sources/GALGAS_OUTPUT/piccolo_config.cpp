...
void routine_actualBuildConfig (const GALGAS_configRegisterMap constinArgument_inConfigRegisterMap,
  ...
  while (enumerator_1067.hasCurrentObject ()) {
    GALGAS_uint var_defaultValue = GALGAS_uint ((uint32_t) 1U).left_shift_operation (enumerator_1067.current_mRegisterWidth (HERE).reader_uint (SOURCE_FILE_AT_LINE (29)) COMMA_SOURCE_FILE_AT_LINE (29)).substract_operation (GALGAS_uint ((uint32_t) 1U), inCompiler COMMA_SOURCE_FILE_AT_LINE (29)) ;
    {
  ...
  while (enumerator_1571.hasCurrentObject ()) {
    ...
    while (enumerator_1599.hasCurrentObject ()) {
      ...
      if (kBoolTrue == test_2) {
        ...
        const enumGalgasBool test_3 = GALGAS_bool (kIsStrictSup, var_fieldSettingMap.reader_count (SOURCE_FILE_AT_LINE (57)).objectCompare (GALGAS_uint ((uint32_t) 0U))).boolEnum () ;
        if (kBoolTrue == test_3) {
  ...
  const enumGalgasBool test_5 = GALGAS_bool (kIsStrictSup, var_notDefinedSetting.reader_count (SOURCE_FILE_AT_LINE (69)).objectCompare (GALGAS_uint ((uint32_t) 0U))).boolEnum () ;
  if (kBoolTrue == test_5) {
...
void routine_buildConfig (const GALGAS_configRegisterMap constinArgument_inConfigRegisterMap,
                          ...
                          COMMA_UNUSED_LOCATION_ARGS) {
  outArgument_outActualConfigurationMap.drop () ; // Release 'out' argument
  ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string::makeEmptyString ().reader_stringByLeftAndRightPadding (GALGAS_uint ((uint32_t) 79U), GALGAS_char (TO_UNICODE (42)) COMMA_SOURCE_FILE_AT_LINE (114)).operator_concat (GALGAS_string ("\n") COMMA_SOURCE_FILE_AT_LINE (114)) COMMA_SOURCE_FILE_AT_LINE (114)) ;
  ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string ("*").operator_concat (GALGAS_string ("MICROCONTROLLER CONFIGURATION").reader_stringByLeftAndRightPadding (GALGAS_uint ((uint32_t) 77U), GALGAS_char (TO_UNICODE (32)) COMMA_SOURCE_FILE_AT_LINE (115)) COMMA_SOURCE_FILE_AT_LINE (115)).operator_concat (GALGAS_string ("*\n") COMMA_SOURCE_FILE_AT_LINE (115)) COMMA_SOURCE_FILE_AT_LINE (115)) ;
  ioArgument_ioListFileContents.dotAssign_operation (GALGAS_string::makeEmptyString ().reader_stringByLeftAndRightPadding (GALGAS_uint ((uint32_t) 79U), GALGAS_char (TO_UNICODE (42)) COMMA_SOURCE_FILE_AT_LINE (116)).operator_concat (GALGAS_string ("\n"
    "\n") COMMA_SOURCE_FILE_AT_LINE (116)) COMMA_SOURCE_FILE_AT_LINE (116)) ;
  const enumGalgasBool test_0 = GALGAS_bool (kIsStrictSup, constinArgument_inConfigDefinitionList.reader_length (SOURCE_FILE_AT_LINE (118)).objectCompare (GALGAS_uint ((uint32_t) 0U))).boolEnum () ;
  if (kBoolTrue == test_0) {
    {
