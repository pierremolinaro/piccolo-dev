...
cPtr_instruction_MNOP (const GGS_luint & argument_0
                                COMMA_LOCATION_ARGS)
:cPtr_instruction (THERE),
mOccurrenceFactor (argument_0) {
}
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  ...
  if (equal) {
    ...
    macroValidPointer (_p) ;
    equal = mOccurrenceFactor._operator_isEqual (_p->mOccurrenceFactor).boolValue () ;
  }
  return equal ;
}
...
method_instructionLength (C_Compiler & _inLexique,
                                GGS_uint & var_cas_outLength COMMA_UNUSED_LOCATION_ARGS) const {
  var_cas_outLength = mOccurrenceFactor.reader_uint (_inLexique COMMA_SOURCE_FILE_AT_LINE (635))._multiply_operation (_inLexique, GGS_uint (true, 2U) COMMA_SOURCE_FILE_AT_LINE (635)) ;
}
...
constructor_new (C_Compiler & /* inLexique */,
  ...
  macroMyNew (result.mPointer, cPtr_instruction_MNOP (argument_0 COMMA_THERE)) ;
  return result ;
}
