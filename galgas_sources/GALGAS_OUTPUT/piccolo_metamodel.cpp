...
elementOf_GGS_elsifPartList::
elementOf_GGS_elsifPartList (const GGS_conditionExpression & argument_0,
                                const GGS_instructionList & argument_1
                                COMMA_LOCATION_ARGS) :
AC_galgas_list::cListElement (THERE),
mCondition (argument_0),
...
bool elementOf_GGS_elsifPartList::
isEqualToObject (const cListElement * inOperand) const {
  bool equal = inOperand == this ;
  if (! equal) {
    const elementOf_GGS_elsifPartList * _p = dynamic_cast <const elementOf_GGS_elsifPartList *> (inOperand) ;
    macroValidPointer (_p) ;
    equal = mCondition._operator_isEqual (_p->mCondition).boolValue ()
         && mInstructionList._operator_isEqual (_p->mInstructionList).boolValue () ;
  }
...
void elementOf_GGS_elsifPartList::
appendForDescription (C_Compiler & _inLexique,
                          C_String & ioString,
                          const sint32 inIndentation
                          COMMA_LOCATION_ARGS) const {
...
GGS_elsifPartList GGS_elsifPartList::
_operator_concat (const GGS_elsifPartList & inOperand) const {
  GGS_elsifPartList result = * this ;
  result._dotAssign_operation (inOperand) ;
  return result ;
}
...
void GGS_elsifPartList::
_dotAssign_operation (const GGS_elsifPartList inOperand) {
  if (_isBuilt () && inOperand._isBuilt ()) {
    if (count () == 0) {
      * this = inOperand ;
    }else{
      if (inOperand.count () > 0) {
        _insulateList () ;
        elementOf_GGS_elsifPartList * p = inOperand.firstObject () ;
        while (p != NULL) {
          macroValidPointer (p) ;
...
GGS_string GGS_elsifPartList::
reader_description (C_Compiler & _inLexique
                    COMMA_LOCATION_ARGS,
                    const sint32 inIndentation) const {
  return _description (_inLexique, "@elsifPartList", inIndentation COMMA_THERE) ;
}
...
GGS_conditionExpression  GGS_elsifPartList::
...
GGS_instructionList  GGS_elsifPartList::
...
/* const */ GGS_conditionExpression  & GGS_elsifPartList::cEnumerator::_mCondition (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
...
/* const */ GGS_instructionList  & GGS_elsifPartList::cEnumerator::_mInstructionList (LOCATION_ARGS) const {
  macroValidPointerThere (mCurrentEnumeratedObject) ;
...
GGS_elsifPartList  GGS_instruction_structured_if::
