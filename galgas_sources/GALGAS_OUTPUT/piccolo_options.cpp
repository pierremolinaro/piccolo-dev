//---------------------------------------------------------------------------*
//                                                                           *
//                        File 'piccolo_options.cpp'                         *
//                 Generated by version GALGAS_BETA_VERSION                  *
//                     august 23th, 2008, at 16h14'27"                       *
//                                                                           *
//---------------------------------------------------------------------------*

//--- START OF USER ZONE 1


//--- END OF USER ZONE 1

//---------------------------------------------------------------------------*

#include "version_libpm.h"
#if LIBPM_VERSION != THE_LIBPM_VERSION
  #error "This file has been compiled with a version of GALGAS different than the version of libpm"
#endif

#include <string.h>

#include "piccolo_options.h"

//--- START OF USER ZONE 2


//--- END OF USER ZONE 2

//---------------------------------------------------------------------------*
//                                                                           *
//                          C O N S T R U C T O R                            *
//                                                                           *
//---------------------------------------------------------------------------*

piccolo_options::piccolo_options (void)
:mStringValue (1 COMMA_HERE) {
  mBoolOptionValues [0] = false ;
  mBoolOptionValues [1] = false ;
  mBoolOptionValues [2] = false ;
  mBoolOptionValues [3] = false ;
  mBoolOptionValues [4] = false ;
  mBoolOptionValues [5] = false ;
  mBoolOptionValues [6] = false ;
  mBoolOptionValues [7] = false ; // Extra unused entry
  mUintOptionValues [0] = 0 ; // Extra unused entry
}

//---------------------------------------------------------------------------*
//                                                                           *
//              G E T    B O O L    O P T I O N S    C O U N T               *
//                                                                           *
//---------------------------------------------------------------------------*

sint32 piccolo_options::getBoolOptionsCount (void) const {
  return 7 ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//               G E T    B O O L    O P T I O N    V A L U E                *
//                                                                           *
//---------------------------------------------------------------------------*

bool piccolo_options::getBoolOptionValue (const sint32 inIndex) const {
  return ((inIndex >= 0) && (inIndex < 7)) ? mBoolOptionValues [inIndex] : false ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//               S E T    B O O L    O P T I O N    V A L U E                *
//                                                                           *
//---------------------------------------------------------------------------*

void piccolo_options::setBoolOptionValue (const sint32 inIndex, const bool inValue) {
  if ((inIndex >= 0) && (inIndex < 7)) {
    mBoolOptionValues [inIndex] = inValue ;
  }
}

//---------------------------------------------------------------------------*
//                                                                           *
//           G E T    B O O L    O P T I O N    C H A R A C T E R            *
//                                                                           *
//---------------------------------------------------------------------------*

char piccolo_options::getBoolOptionChar (const sint32 inIndex) const {
  const char kCharArray [8] = {'S', 'L', '\0', 'D', '\0', '\0', 'O', '\0'} ;
  return ((inIndex >= 0) && (inIndex < 7)) ? kCharArray [inIndex] : '\0' ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//              G E T    B O O L    O P T I O N    S T R I N G               *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::getBoolOptionString (const sint32 inIndex) const {
  const char * kStringArray [8] = {"asm",
    "list",
    "baseline",
    "device-list",
    "midrange",
    "pic18",
    "optimize",
    ""} ;
  return ((inIndex >= 0) && (inIndex < 7)) ? kStringArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//         G E T    B O O L    O P T I O N    D E S C R I P T I O N          *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::getBoolOptionDescription (const sint32 inIndex) const {
  const char * kDescriptionArray [8] = {"Generate also an assembly language text file (with .asm extension)",
    "Output a list file",
    "Output the list of supported baseline devices",
    "Output the list of supported devices",
    "Output the list of supported mid-range devices",
    "Output the list of supported pic18 devices",
    "Perform code optimizations",
    ""} ;
  return ((inIndex >= 0) && (inIndex < 7)) ? kDescriptionArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//    G E T    B O O L    O P T I O N    V A L U E    F R O M    K E Y S     *
//                                                                           *
//---------------------------------------------------------------------------*

bool piccolo_options::
boolOptionValueFromKeys (const char * inModuleName,
                         const char * inOptionName,
                         bool * outFound) const {
  const char * kKeyArray [8] = {"generateAssembly",
    "ouputListingFile",
    "outputBaselineDeviceList",
    "outputDeviceList",
    "outputMidrangeDeviceList",
    "outputPic18DeviceList",
    "performOptimizations",
    ""} ;
  bool result = false ;
  bool found = strcmp (inModuleName, "piccolo_options") == 0 ;
  if (found) {
    found = false ;
    sint32 index = 0 ;
    while ((index < 7) && ! found) {
      found = strcmp (inOptionName, kKeyArray [index]) == 0 ;
      if (found) {
        result = mBoolOptionValues [index] ;
      }
      index ++ ;
    }
  }
  if (outFound != NULL) {
    * outFound = found ;
  }
  return result ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//              G E T    U I N T    O P T I O N S    C O U N T               *
//                                                                           *
//---------------------------------------------------------------------------*

sint32 piccolo_options::getUintOptionsCount (void) const {
  return 0 ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//               G E T    U I N T    O P T I O N    V A L U E                *
//                                                                           *
//---------------------------------------------------------------------------*

uint32 piccolo_options::getUintOptionValue (const sint32 inIndex) const {
  return ((inIndex >= 0) && (inIndex < 0)) ? mUintOptionValues [inIndex] : 0 ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//      G E T    U I N T    O P T I O N    D E F A U L T    V A L U E        *
//                                                                           *
//---------------------------------------------------------------------------*

uint32 piccolo_options::getUintOptionDefaultValue (const sint32 inIndex) const {
  static const uint32 kDefaultValues [1] = {
  0} ;
  return ((inIndex >= 0) && (inIndex < 0)) ? kDefaultValues [inIndex] : 0 ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//               S E T    U I N T    O P T I O N    V A L U E                *
//                                                                           *
//---------------------------------------------------------------------------*

void piccolo_options::setUintOptionValue (const sint32 inIndex, const uint32 inValue) {
  if ((inIndex >= 0) && (inIndex < 0)) {
    mUintOptionValues [inIndex] = inValue ;
  }
}

//---------------------------------------------------------------------------*
//                                                                           *
//           G E T    U I N T    O P T I O N    C H A R A C T E R            *
//                                                                           *
//---------------------------------------------------------------------------*

char piccolo_options::getUintOptionChar (const sint32 inIndex) const {
  static const char kCharArray [1] = {'\0'} ;
  return ((inIndex >= 0) && (inIndex < 0)) ? kCharArray [inIndex] : '\0' ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//              G E T    U I N T    O P T I O N    S T R I N G               *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::getUintOptionString (const sint32 inIndex) const {
  static const char * kStringArray [1] = {""} ;
  return ((inIndex >= 0) && (inIndex < 0)) ? kStringArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//         G E T   U I N T    O P T I O N    D E S C R I P T I O N           *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::getUintOptionDescription (const sint32 inIndex) const {
  static const char * kDescriptionArray [1] = {""} ;
  return ((inIndex >= 0) && (inIndex < 0)) ? kDescriptionArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//    G E T    U I N T    O P T I O N    V A L U E    F R O M    K E Y S     *
//                                                                           *
//---------------------------------------------------------------------------*

uint32 piccolo_options::
uintOptionValueFromKeys (const char * inModuleName,
                         const char * inOptionName,
                         bool * outFound) const {
  static const char * kKeyArray [1] = {""} ;
  uint32 result = 0 ;
  bool found = strcmp (inModuleName, "piccolo_options") == 0 ;
  if (found) {
    found = false ;
    sint32 index = 0 ;
    while ((index < 0) && ! found) {
      found = strcmp (inOptionName, kKeyArray [index]) == 0 ;
      if (found) {
        result = mUintOptionValues [index] ;
      }
      index ++ ;
    }
  }
  if (outFound != NULL) {
    * outFound = found ;
  }
  return result ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//            G E T    S T R I N G    O P T I O N S    C O U N T             *
//                                                                           *
//---------------------------------------------------------------------------*

sint32 piccolo_options::
getStringOptionsCount (void) const {
  return 4 ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//            G E T    S T R I N G    O P T I O N S    V A L U E             *
//                                                                           *
//---------------------------------------------------------------------------*

C_String piccolo_options::
getStringOptionValue (const sint32 inIndex) const {
  return ((inIndex >= 0) && (inIndex < 4)) ? mStringOptionValues [inIndex] : C_String () ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//            S E T    S T R I N G    O P T I O N S    V A L U E             *
//                                                                           *
//---------------------------------------------------------------------------*

void piccolo_options::
setStringOptionValue (const sint32 inIndex, const C_String & inValue) {
  if ((inIndex >= 0) && (inIndex < 4)) {
     mStringOptionValues [inIndex] = inValue ;
  }
}

//---------------------------------------------------------------------------*
//                                                                           *
//              G E T    S T R I N G    O P T I O N    C H A R               *
//                                                                           *
//---------------------------------------------------------------------------*

char piccolo_options::
getStringOptionChar (const sint32 inIndex) const {
  static const char kCharArray [5] = {'F', 'E', 'M', 'R', '\0'} ;
  return ((inIndex >= 0) && (inIndex < 4)) ? kCharArray [inIndex] : '\0' ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//            G E T    S T R I N G    O P T I O N    S T R I N G             *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::
getStringOptionString (const sint32 inIndex) const {
  static const char * kStringArray [5] = {"configuration",
    "export",
    "memory",
    "registers",
    ""} ;
  return ((inIndex >= 0) && (inIndex < 4)) ? kStringArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//       G E T    S T R I N G    O P T I O N    D E S C R I P T I O N        *
//                                                                           *
//---------------------------------------------------------------------------*

const char * piccolo_options::
getStringOptionDescription (const sint32 inIndex) const {
  static const char * kDescriptionArray [5] = {"Output the configuration register description of the device named by string",
    "Export device definition files to the directory named by string",
    "Output the RAM, ROM and EEPROM description of the device named by string",
    "Output the special register description of the device named by string",
    ""} ;
  return ((inIndex >= 0) && (inIndex < 4)) ? kDescriptionArray [inIndex] : "" ;
}

//---------------------------------------------------------------------------*
//                                                                           *
//   G E T    S T R I N G    O P T I O N    V A L U E   F R O M   K E Y S    *
//                                                                           *
//---------------------------------------------------------------------------*

C_String piccolo_options::
stringOptionValueFromKeys (const char * inModuleName,
                           const char * inOptionName,
                           bool * outFound) const {
  static const char * kKeyArray [5] = {"configDescription",
    "exportDeviceFiles",
    "memoryDescription",
    "registerDescription",
    ""} ;
  C_String result ;
  bool found = strcmp (inModuleName, "piccolo_options") == 0 ;
  if (found) {
    found = false ;
    sint32 index = 0 ;
    while ((index < 4) && ! found) {
      found = strcmp (inOptionName, kKeyArray [index]) == 0 ;
      if (found) {
        result = mStringOptionValues [index] ;
      }
      index ++ ;
    }
  }
  if (outFound != NULL) {
    * outFound = found ;
  }
  return result ;
}

//---------------------------------------------------------------------------*

