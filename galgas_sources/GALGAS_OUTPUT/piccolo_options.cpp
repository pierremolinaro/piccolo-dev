...
char piccolo_options::getBoolOptionChar (const sint32 inIndex) const {
  const char kCharArray [5] = {'S', 'D', 'L', 'O', '\0'} ;
...
const char * piccolo_options::getBoolOptionDescription (const sint32 inIndex) const {
  const char * kDescriptionArray [5] = {"Generate also an assembly language text file (with .asm extension)",
...
boolOptionValueFromKeys (const char * inModuleName,
                         ...
                         bool * outFound) const {
  const char * kKeyArray [5] = {"generateAssembly",
    "ouputDeviceList",
    "ouputListingFile",
