...
:mStringValue (1 COMMA_HERE) {
  ...
  mBoolOptionValues [1] = false ; // Extra unused entry
...
bool piccolo_options::getBoolOptionValue (const sint32 inIndex) const {
  return ((inIndex >= 0) && (inIndex < 1)) ? mBoolOptionValues [inIndex] : false ;
}
...
const char * piccolo_options::getBoolOptionDescription (const sint32 inIndex) const {
  const char * kDescriptionArray [2] = {"Generate assembly language text file, instead of hex file",
    ""} ;
...
boolOptionValueFromKeys (const char * inModuleName,
                         ...
                         bool * outFound) const {
  const char * kKeyArray [2] = {"generateAssembly",
    ""} ;
  bool result = false ;
