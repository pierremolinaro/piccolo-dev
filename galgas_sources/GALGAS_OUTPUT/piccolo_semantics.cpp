...
void routine_analyze (C_Compiler & _inLexique,
  ...
  var_cas_lowInterruptIsFast = GGS_bool (true, false) ;
  {
    ...
    while (((operand_4688 = enumerator_4688.nextObject ()))) {
      ...
      if (((operand_4688->mInterruptName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (101)))._operator_isEqual (GGS_string (true, "high"))).isBuiltAndTrue ()) {
        var_cas_highInterruptIsFast = operand_4688->mFastReturn ;
        ...
        var_cas_lowInterruptIsFast = operand_4688->mFastReturn ;
  ...
  {
    GGS_ramDefinitionList::cEnumerator enumerator_6016 (var_cas_inPiccoloModel.reader_mRamDefinitionList (_inLexique COMMA_SOURCE_FILE_AT_LINE (132)), true) ;
    const GGS_ramDefinitionList::cElement * operand_6016 = NULL ;
    while (((operand_6016 = enumerator_6016.nextObject ()))) {
      macroValidPointer (operand_6016) ;
      {
        GGS_declarationInRamList::cEnumerator enumerator_6067 (operand_6016->mDeclaration, true) ;
        const GGS_declarationInRamList::cElement * operand_6067 = NULL ;
        while (((operand_6067 = enumerator_6067.nextObject ()))) {
          macroValidPointer (operand_6067) ;
          const GGS_declarationInRam  _temp_6103 = operand_6067->mDeclaration ;
          if (_temp_6103._isBuilt ()) {
            typeCategoryMethod__declarationInRam__handleDeclaration _method = findCategoryMethod__declarationInRam__handleDeclaration (_temp_6103._galgasObjectRunTimeInfo ()) ;
            if (_method != NULL) {
              _method (_inLexique, _temp_6103 (HERE), var_cas_ramBank, var_cas_registerTable, operand_6016->mBankName COMMA_SOURCE_FILE_AT_LINE (134)) ;
            }
          }
        }
      }
  ...
  if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "ouputListingFile" COMMA_SOURCE_FILE_AT_LINE (137)))).isBuiltAndTrue ()) {
    ...
    var_cas_listFileContents.writeString ("\n"
      "\n") ;
    {
      GGS_registerTable::cEnumerator enumerator_6511 (var_cas_registerTable, true) ;
      const GGS_registerTable::cElement * operand_6511 = NULL ;
      while (((operand_6511 = enumerator_6511.nextObject ()))) {
        macroValidPointer (operand_6511) ;
        if (((var_cas_piccoloDeviceModel.reader_mRegisterTable (_inLexique COMMA_SOURCE_FILE_AT_LINE (142)).reader_hasKey (_inLexique, operand_6511->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (142)) COMMA_SOURCE_FILE_AT_LINE (142)))._operator_not ()).isBuiltAndTrue ()) {
          var_cas_listFileContents._dotAssign_operation (operand_6511->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (143)).reader_stringByLeftPadding (_inLexique, GGS_uint (true, 40U), GGS_char (true, ' ') COMMA_SOURCE_FILE_AT_LINE (143))) ;
          var_cas_listFileContents.writeString (" ") ;
          GGS_uint  var_cas_address ;
          const GGS_uintlist  _temp_6731 = operand_6511->mInfo.mRegisterAddressList ;
          if (_temp_6731._isBuilt ()) {
            _temp_6731 (HERE)->method_first (_inLexique, var_cas_address COMMA_SOURCE_FILE_AT_LINE (144)) ;
          }
          var_cas_listFileContents._dotAssign_operation (var_cas_address.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (145))) ;
          var_cas_listFileContents._dotAssign_operation (operand_6511->mInfo.mBitDefinitionString) ;
          var_cas_listFileContents.writeString ("\n") ;
        }
      }
    }
  ...
  if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "ouputListingFile" COMMA_SOURCE_FILE_AT_LINE (179)))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_8107 = var_cas_listFileContents ;
    if (_temp_8107._isBuilt ()) {
      _temp_8107 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_listFile COMMA_SOURCE_FILE_AT_LINE (181)) ;
    }
  }
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_8565 = var_cas_contents ;
    if (_temp_8565._isBuilt ()) {
      _temp_8565 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_hexDestinationFile COMMA_SOURCE_FILE_AT_LINE (194)) ;
    }
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (196)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_9117 = var_cas_assemblyCode ;
      if (_temp_9117._isBuilt ()) {
        _temp_9117 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_asmDestinationFile COMMA_SOURCE_FILE_AT_LINE (207)) ;
      }
    }
  }else{
    GGS_string var_cas_hexDestinationFile ;
