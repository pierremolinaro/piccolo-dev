...
void routine_analyze (C_Compiler & _inLexique,
  ...
  {
    GGS_interruptDefinitionList::cEnumerator enumerator_1490 (var_cas_inPiccoloModel.reader_mInterruptDefinitionList (_inLexique COMMA_SOURCE_FILE_AT_LINE (25)), true) ;
    const GGS_interruptDefinitionList::cElement * operand_1490 = NULL ;
    while (((operand_1490 = enumerator_1490.nextObject ()))) {
      macroValidPointer (operand_1490) ;
      if (((operand_1490->mInterruptName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (26)))._operator_isEqual (GGS_string (true, "high"))).isBuiltAndTrue ()) {
        if ((var_cas_hasHighInterrupt).isBuiltAndTrue ()) {
          operand_1490->mInterruptName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "Only one 'high' interrupt routine is allowed") COMMA_SOURCE_FILE_AT_LINE (28)) ;
        }
        var_cas_hasHighInterrupt = GGS_bool (true, true) ;
      }else if (((operand_1490->mInterruptName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (31)))._operator_isEqual (GGS_string (true, "low"))).isBuiltAndTrue ()) {
        if ((var_cas_hasLowInterrupt).isBuiltAndTrue ()) {
          operand_1490->mInterruptName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "Only one 'low' interrupt routine is allowed") COMMA_SOURCE_FILE_AT_LINE (33)) ;
        }
        var_cas_hasLowInterrupt = GGS_bool (true, true) ;
      }else{
        operand_1490->mInterruptName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "An interrupt routine should be named 'low' or 'high'") COMMA_SOURCE_FILE_AT_LINE (37)) ;
      }
    }
  }
  ...
  {
    GGS_ramDefinitionList::cEnumerator enumerator_2260 (var_cas_inPiccoloModel.reader_mRamDefinitionList (_inLexique COMMA_SOURCE_FILE_AT_LINE (43)), true) ;
    const GGS_ramDefinitionList::cElement * operand_2260 = NULL ;
    while (((operand_2260 = enumerator_2260.nextObject ()))) {
      macroValidPointer (operand_2260) ;
      GGS_uint  var_cas_lastAddressPlusOne ;
      GGS_uint  var_cas_firstFreeAddress ;
      const GGS_ramBankTable  _temp_2393 = var_cas_ramBank ;
      if (_temp_2393._isBuilt ()) {
        _temp_2393 (HERE)->method_searchKey (_inLexique, operand_2260->mBankName, var_cas_firstFreeAddress, var_cas_lastAddressPlusOne COMMA_SOURCE_FILE_AT_LINE (47)) ;
      }
      {
        GGS_declarationInRamList::cEnumerator enumerator_2467 (operand_2260->mDeclaration, true) ;
        const GGS_declarationInRamList::cElement * operand_2467 = NULL ;
        while (((operand_2467 = enumerator_2467.nextObject ()))) {
          macroValidPointer (operand_2467) ;
          if (((operand_2467->mSize.reader_uint (_inLexique COMMA_SOURCE_FILE_AT_LINE (49)))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
            operand_2467->mSize.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "zero size is not allowed") COMMA_SOURCE_FILE_AT_LINE (50)) ;
          }
          ...
          var_cas_registerTable.modifier_insertKey (_inLexique, operand_2467->mName, GGS_uintlist ::constructor_listWithValue (var_cas_firstFreeAddress), operand_2467->mSize.reader_uint (_inLexique COMMA_SOURCE_FILE_AT_LINE (53)), var_cas_bitSliceTable COMMA_SOURCE_FILE_AT_LINE (53)) ;
          var_cas_firstFreeAddress = var_cas_firstFreeAddress._add_operation (_inLexique, operand_2467->mSize.reader_uint (_inLexique COMMA_SOURCE_FILE_AT_LINE (54)) COMMA_SOURCE_FILE_AT_LINE (54)) ;
          var_cas_ramBank.modifier_setMFirstFreeAddressForKey (_inLexique, var_cas_firstFreeAddress, operand_2260->mBankName.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (55)) COMMA_SOURCE_FILE_AT_LINE (55)) ;
          if (((var_cas_firstFreeAddress)._operator_strictSup (var_cas_lastAddressPlusOne)).isBuiltAndTrue ()) {
            operand_2467->mName.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((((GGS_string (true, "overflow in '"))._operator_concat (operand_2260->mBankName))._operator_concat (GGS_string (true, "' ram bank for '")))._operator_concat (operand_2467->mName))._operator_concat (GGS_string (true, "' declaration")) COMMA_SOURCE_FILE_AT_LINE (57)) ;
          }
        }
      }
    }
  ...
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (78)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_4149 = var_cas_assemblyCode ;
      if (_temp_4149._isBuilt ()) {
        _temp_4149 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_destinationFile COMMA_SOURCE_FILE_AT_LINE (88)) ;
      }
    }else{
      ...
      const GGS_string _temp_4471 = var_cas_contents ;
      if (_temp_4471._isBuilt ()) {
        _temp_4471 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_destinationFile COMMA_SOURCE_FILE_AT_LINE (97)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
