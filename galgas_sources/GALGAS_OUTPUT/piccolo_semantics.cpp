...
void routine_analyze (C_Compiler & _inLexique,
  ...
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (78)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_4154 = var_cas_assemblyCode ;
      if (_temp_4154._isBuilt ()) {
        _temp_4154 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_destinationFile COMMA_SOURCE_FILE_AT_LINE (88)) ;
      }
    }else{
      ...
      const GGS_string _temp_4476 = var_cas_contents ;
      if (_temp_4476._isBuilt ()) {
        _temp_4476 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_destinationFile COMMA_SOURCE_FILE_AT_LINE (97)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
