...
void routine_analyze (C_Compiler & _inLexique,
  ...
  if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "ouputListFile" COMMA_SOURCE_FILE_AT_LINE (137)))).isBuiltAndTrue ()) {
    ...
    var_cas_listFileContents.writeString ("\n"
      "\n") ;
    {
      GGS_registerTable::cEnumerator enumerator_6508 (var_cas_registerTable, true) ;
      const GGS_registerTable::cElement * operand_6508 = NULL ;
      while (((operand_6508 = enumerator_6508.nextObject ()))) {
        macroValidPointer (operand_6508) ;
        if (((var_cas_piccoloDeviceModel.reader_mRegisterTable (_inLexique COMMA_SOURCE_FILE_AT_LINE (142)).reader_hasKey (_inLexique, operand_6508->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (142)) COMMA_SOURCE_FILE_AT_LINE (142)))._operator_not ()).isBuiltAndTrue ()) {
          var_cas_listFileContents._dotAssign_operation (operand_6508->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (143)).reader_stringByLeftPadding (_inLexique, GGS_uint (true, 40U), GGS_char (true, ' ') COMMA_SOURCE_FILE_AT_LINE (143))) ;
          var_cas_listFileContents.writeString (" ") ;
          GGS_uint  var_cas_address ;
          const GGS_uintlist  _temp_6728 = operand_6508->mInfo.mRegisterAddressList ;
          if (_temp_6728._isBuilt ()) {
            _temp_6728 (HERE)->method_first (_inLexique, var_cas_address COMMA_SOURCE_FILE_AT_LINE (144)) ;
          }
          var_cas_listFileContents._dotAssign_operation (var_cas_address.reader_hexString (_inLexique COMMA_SOURCE_FILE_AT_LINE (145))) ;
          var_cas_listFileContents._dotAssign_operation (operand_6508->mInfo.mBitDefinitionString) ;
          var_cas_listFileContents.writeString ("\n") ;
        }
      }
    }
  ...
  if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "ouputListFile" COMMA_SOURCE_FILE_AT_LINE (179)))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_8101 = var_cas_listFileContents ;
    if (_temp_8101._isBuilt ()) {
      _temp_8101 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_listFile COMMA_SOURCE_FILE_AT_LINE (181)) ;
    }
  }
  if (((GGS_uint ::constructor_errorCount (_inLexique COMMA_HERE))._operator_isEqual (GGS_uint (true, 0U))).isBuiltAndTrue ()) {
    ...
    const GGS_string _temp_8559 = var_cas_contents ;
    if (_temp_8559._isBuilt ()) {
      _temp_8559 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_hexDestinationFile COMMA_SOURCE_FILE_AT_LINE (194)) ;
    }
    if ((GGS_bool (true, _inLexique.boolOptionValueFromKeys ("piccolo_options", "generateAssembly" COMMA_SOURCE_FILE_AT_LINE (196)))).isBuiltAndTrue ()) {
      ...
      const GGS_string _temp_9111 = var_cas_assemblyCode ;
      if (_temp_9111._isBuilt ()) {
        _temp_9111 (HERE)->method_writeToFileWhenDifferentContents (_inLexique, var_cas_asmDestinationFile COMMA_SOURCE_FILE_AT_LINE (207)) ;
      }
    }
  }else{
    GGS_string var_cas_hexDestinationFile ;
