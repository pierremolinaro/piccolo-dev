...
void routine_analyze_5F_ram_5F_sections (const GALGAS_string constinArgument_inListingTitle,
  ...
  cEnumerator_ramDefinitionList enumerator_2455 (constinArgument_inRamDefinitionList, kEnumeration_up) ;
  //--- simple foreach instruction
  while (enumerator_2455.hasCurrentObject ()) {
    //-------- @foreachInstructionForGeneration
    cEnumerator_declarationInRamList enumerator_2483 (enumerator_2455.current_mDeclaration (HERE), kEnumeration_up) ;
    //--- simple foreach instruction
    while (enumerator_2483.hasCurrentObject ()) {
      //-------- @methodCallInstructionForGeneration
      callCategoryMethod_handleDeclaration ((const cPtr_declarationInRam *) enumerator_2483.current_mDeclarationInRAM (HERE).ptr (), ioArgument_ioRamBankTable, ioArgument_ioAllRegisters, enumerator_2455.current_mBankName (HERE), inCompiler COMMA_SOURCE_FILE_AT_LINE (61)) ;
  ...
  if (kBoolTrue == test_0) {
    ...
    //-------- @foreachInstructionForGeneration
    cEnumerator_registerTable enumerator_3033 (ioArgument_ioAllRegisters, kEnumeration_up) ;
    //--- simple foreach instruction
    while (enumerator_3033.hasCurrentObject ()) {
      //-------- @ifInstructionForGeneration
      const enumGalgasBool test_1 = constinArgument_inSpecialRegisterTable.reader_hasKey (enumerator_3033.current_lkey (HERE).reader_string (SOURCE_FILE_AT_LINE (70)) COMMA_SOURCE_FILE_AT_LINE (70)).operator_not (SOURCE_FILE_AT_LINE (70)).boolEnum () ;
      if (kBoolTrue == test_1) {
        //-------- @concatInstructionForGeneration
        ioArgument_ioListFileContents.dotAssign_operation (enumerator_3033.current_lkey (HERE).reader_string (SOURCE_FILE_AT_LINE (71)).reader_stringByLeftPadding (GALGAS_uint ((PMUInt32) 40U), GALGAS_char (TO_UNICODE (32)) COMMA_SOURCE_FILE_AT_LINE (71)).operator_concat (GALGAS_string (" ") COMMA_SOURCE_FILE_AT_LINE (71)) COMMA_SOURCE_FILE_AT_LINE (71)) ;
        //-------- @foreachInstructionForGeneration
        cEnumerator_uintlist enumerator_3229 (enumerator_3033.current_mRegisterAddressList (HERE), kEnumeration_up) ;
        //--- simple foreach instruction
        while (enumerator_3229.hasCurrentObject ()) {
          //-------- @concatInstructionForGeneration
          ioArgument_ioListFileContents.dotAssign_operation (enumerator_3229.current_mValue (HERE).reader_hexString (SOURCE_FILE_AT_LINE (73)) COMMA_SOURCE_FILE_AT_LINE (73)) ;
        ...
        //-------- @concatInstructionForGeneration
        ioArgument_ioListFileContents.dotAssign_operation (enumerator_3033.current_mBitDefinitionString (HERE).operator_concat (GALGAS_string ("\n") COMMA_SOURCE_FILE_AT_LINE (76)) COMMA_SOURCE_FILE_AT_LINE (76)) ;
      }
...
static void categoryMethod_byteDeclarationInRam_handleDeclaration (const cPtr_declarationInRam * inObject,
  ...
  //-------- @methodCallInstructionForGeneration
  GALGAS_uint joker_1061_0 ; // Joker input parameter
  ioArgument_ioRamBank.method_searchKey (constinArgument_inCurrentRamBank, joker_1061_0, var_firstFreeAddress, var_lastAddressPlusOne, var_mirrorOffsetList, inCompiler COMMA_SOURCE_FILE_AT_LINE (29)) ;
  //-------- END @methodCallInstructionForGeneration
  //-------- @ifInstructionForGeneration
  ...
  //-------- @foreachInstructionForGeneration
  cEnumerator_uintlist enumerator_1295 (var_mirrorOffsetList, kEnumeration_up) ;
  //--- simple foreach instruction
  while (enumerator_1295.hasCurrentObject ()) {
    //-------- @plusEqualnstructionForGeneration
    var_addressList.addAssign_operation (var_firstFreeAddress.add_operation (enumerator_1295.current_mValue (HERE), inCompiler COMMA_SOURCE_FILE_AT_LINE (35)) COMMA_SOURCE_FILE_AT_LINE (35)) ;
