...
void routine_analyze_ram_sections (C_Compiler & inLexique,
  ...
  const GGS_bool cond_4863 = gOption_piccolo_5F_options_ouputListingFile.value () ;
  if (cond_4863.isBuiltAndTrue ()) {
    ...
    var_cas_ioListFileContents.appendCString (" Address\n") ;
    {
      ...
      while (((operand_4424 = enumerator_4424.nextObject ()))) {
        ...
        const GGS_bool cond_4801 = (var_cas_inSpecialRegisterTable.reader_hasKey (operand_4424->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (101)))).operator_not () ;
        if (cond_4801.isBuiltAndTrue ()) {
          var_cas_ioListFileContents.dotAssign_operation (operand_4424->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (102)).reader_stringByLeftPadding (GGS_uint (40U), GGS_char (TO_UNICODE (' ')))) ;
          var_cas_ioListFileContents.appendCString (" ") ;
          {
